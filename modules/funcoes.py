...
def mutacao(w,k,part): 
    ...
    #print(f"part inicial {part.view_nice(w,k)}")
    ...
    #print(f"part final {part.view_nice(w,k)}")
...
def reproducao(mae, pai, w, k):
    ...
    #print(f"pai = {particao_pai.view_nice(w,k)}\nmãe = {particao_mae.view_nice(w,k)}\nmagica pai: {magica_pai}\nmagica mae {magica_mae}")
    ...
    if any(map(lambda x: x != -1, magica_pai)):
        ...
        if possivel:
            ...
            #print(f"filho: {filho.view_nice(w,k)}\nmagica filho: {heranca}")
